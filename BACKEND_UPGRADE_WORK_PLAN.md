...
- **문제**: 실제 게임 서버와 연동되지 않음

### 작업 내용
- [ ] **2.1.1** 게임 서버에서 핸드 완료 시 이벤트 발행 구조 설계
- [ ] **2.1.2** Redis pub/sub으로 핸드 결과 전송
- [ ] **2.1.3** admin-backend에서 핸드 결과 수신 및 분석
- [ ] **2.1.4** 의심 패턴 발견 시 알림 발송
- [ ] **2.1.5** 테스트 실행
- [ ] **2.1.6** 수정 검증 완료

### 완료 확인
- [ ] **단계 2.1 완료** (날짜: ______, 검증: ____ tests passed)
...
### 작업 내용
- [ ] **2.2.1** 플레이어 액션 타이밍 데이터 수집
- [ ] **2.2.2** 액션 패턴 데이터 Redis 저장
- [ ] **2.2.3** admin-backend에서 패턴 분석
- [ ] **2.2.4** 봇 의심 시 자동 플래그 처리
- [ ] **2.2.5** 테스트 실행
- [ ] **2.2.6** 수정 검증 완료

### 완료 확인
- [ ] **단계 2.2 완료** (날짜: ______, 검증: ____ tests passed)
...
### 완료 확인
- [ ] **단계 2.3 완료** (날짜: ______, 검증: ____ tests passed)
...
### 현재 상태
- AutoBan 서비스 구현 완료
- **문제**: 탐지 시스템과 자동 연동 미구현

### 작업 내용
- [ ] **2.4.1** 탐지 결과 → 자동 밴 트리거 연결
- [ ] **2.4.2** 밴 임계값 설정 (config.py)
- [ ] **2.4.3** 관리자 알림 연동
- [ ] **2.4.4** 테스트 실행
- [ ] **2.4.5** 수정 검증 완료

### 완료 확인
- [ ] **단계 2.4 완료** (날짜: ______, 검증: ____ tests passed)
...
**우선순위**: 🟠 P1
**예상 시간**: 60분
**파일**: `backend/app/services/hand_history.py` (신규)

### 현재 상태
- 핸드 기록이 메모리에만 존재
- **문제**: 서버 재시작 시 기록 손실, 부정 행위 분석 불가

### 작업 내용
- [ ] **2.5.1** HandHistory 모델 생성 (PostgreSQL)
- [ ] **2.5.2** 핸드 완료 시 DB 저장 로직 추가
- [ ] **2.5.3** 핸드 조회 API 추가
- [ ] **2.5.4** Alembic 마이그레이션 생성
- [ ] **2.5.5** 테스트 실행
- [ ] **2.5.6** 수정 검증 완료

### 완료 확인
- [ ] **단계 2.5 완료** (날짜: ______, 검증: ____ tests passed)
